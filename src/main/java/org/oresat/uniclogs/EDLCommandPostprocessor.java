...
public class EDLCommandPostprocessor implements CommandPostprocessor {
    ...
    public byte[] process(PreparedCommand pc) {
        ...
        LOG.debug("Using Header: " + byteArrayToHexString(header));

        // Get the payload
        byte[] payload = pc.getBinary();
        LOG.debug("Generated Command Payload: " + byteArrayToHexString(payload));

        // Get the salt
        Integer serialNumber = 1024; // TODO: Load this from somewhere externally
        byte[] salt = ByteArrayUtils.encodeInt(serialNumber);
        LOG.debug("Generated Salt as a Serial Number: " + ByteArrayUtils.decodeInt(salt, 0) + ": " + byteArrayToHexString(salt));

        // Generate the HMAC Key
        String secret = "assf1234"; // TODO: Load this from somewhere externally
